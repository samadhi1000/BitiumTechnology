...
export default function OrderFormComponent({ hideNavbar = false }: { hideNavbar?: boolean }) {
  ...
  return (
    <div className="w-full">
      {/* Top Navbar Header (Shown when not embedded or as requested) */}
      {!hideNavbar && (
      ...
      <div className="max-w-2xl mx-auto">
        {/* Helper Action Bar when embedded */}
        {hideNavbar && (
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <button
              type="button"
              onClick={copyWhatsAppText}
              className="inline-flex items-center gap-1.5 px-3.5 py-2 text-xs font-bold rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white shadow-md transition-all cursor-pointer shrink-0"
            >
              {copied ? <Check className="w-3.5 h-3.5" /> : <Copy className="w-3.5 h-3.5" />}
              {copied ? "Copied Format!" : "Copy for WhatsApp"}
            </button>
