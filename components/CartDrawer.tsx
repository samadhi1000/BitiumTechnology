      ...
      {isCartOpen && (
        <>
          ...
          <motion.div
            ...
            className="fixed right-0 top-0 bottom-0 z-[100] w-full max-w-lg bg-zinc-950 border-l border-zinc-900 shadow-2xl flex flex-col h-full overflow-hidden"
          >
            ...
            <AnimatePresence>
              {showSuccess && (
                <motion.div
                  ...
                  className="absolute inset-0 z-50 bg-zinc-950/95 flex flex-col items-center justify-center p-8 text-center space-y-6"
                >
                  <motion.div 
                    initial={{ scale: 0 }}
                    animate={{ scale: 1 }}
                  ...
                  <div className="text-[11px] text-zinc-600 bg-zinc-900 border border-zinc-800 rounded-xl px-4 py-3 max-w-xs leading-normal">
                    <p className="font-bold text-zinc-400 mb-1 flex items-center gap-1.5 justify-center">
            ...
            {/* Drawer Header */}
            <div className="p-5 sm:p-6 border-b border-zinc-900 flex items-center justify-between bg-zinc-950">
              <div className="flex items-center gap-3">
                <div className="relative p-2 rounded-xl bg-violet-600/10 border border-violet-500/20 text-violet-400">
                  <ShoppingBag size={20} />
                  {items.length > 0 && (
                    <span className="absolute -top-1 -right-1 flex h-4 w-4 items-center justify-center rounded-full bg-violet-600 text-[9px] font-bold text-white ring-2 ring-zinc-950">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-500 font-medium">Verify your items & finish order checkout</p>
                </div>
              </div>
              <button 
                onClick={closeCart}
                className="p-2 rounded-xl bg-zinc-900 hover:bg-zinc-800 border border-zinc-850 hover:border-zinc-700 text-zinc-400 hover:text-zinc-200 transition-all"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex-1 overflow-y-auto divide-y divide-zinc-900 scrollbar-thin p-5 sm:p-6 space-y-8">
              ...
              {items.length === 0 ? (
                <div className="py-16 text-center space-y-5">
                  <div className="w-16 h-16 bg-zinc-900/50 border border-zinc-850 rounded-full flex items-center justify-center mx-auto text-zinc-600">
                    <ShoppingBag size={28} />
                  </div>
                  <div className="space-y-1.5">
                    <h4 className="font-extrabold text-sm text-zinc-300">Your cart is empty</h4>
                    <p className="text-xs text-zinc-500 max-w-[240px] mx-auto leading-normal">
                ...
                <div className="space-y-4">
                  <span className="text-[10px] font-black text-violet-400 uppercase tracking-widest block mb-2">Selected Items</span>
                  {items.map((item) => (
                    <div 
                      key={item.id}
                      className="flex gap-4 p-4 rounded-2xl border border-zinc-900 bg-zinc-900/10 hover:bg-zinc-900/20 transition-all relative group overflow-hidden"
                    >
                      ...
                      <div className="flex-1 min-w-0 pr-4">
                        ...
                        <div className="flex items-center justify-between mt-3">
                          ...
                          {/* Quantity Selector */}
                          <div className="flex items-center bg-zinc-950/80 border border-zinc-900 rounded-lg p-0.5 scale-90 origin-right">
                            <button
                              onClick={() => updateQuantity(item.id, item.quantity - 1)}
                              className="w-6 h-6 flex items-center justify-center rounded hover:bg-zinc-800 text-xs font-black text-zinc-400 transition-colors"
                            >
                              <Minus size={10} />
                            </button>
                            <span className="w-8 text-center text-[11px] font-bold text-zinc-200">
                              {item.quantity}
                            </span>
                            <button
                              onClick={() => updateQuantity(item.id, item.quantity + 1)}
              ...
              {items.length > 0 && (
                <div className="space-y-4 pt-6 border-t border-zinc-900">
                  <span className="text-[10px] font-black text-violet-400 uppercase tracking-widest block">Customer Details</span>
                  ...
                  <form onSubmit={handleCheckout} className="space-y-4">
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                        <User size={11} className="text-zinc-500" /> Full Name <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                        ...
                        className={`w-full p-3 rounded-xl border bg-zinc-900/40 text-sm text-white placeholder-zinc-600 outline-none transition-all ${
                          errors.name 
                            ...
                            : 'border-zinc-850 focus:border-violet-500 focus:ring-1 focus:ring-violet-500/30'
                        }`}
                      />
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                        <Phone size={11} className="text-zinc-500" /> WhatsApp / Mobile Number <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="tel"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                        <MapPin size={11} className="text-zinc-500" /> Delivery Address <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                        <MapPin size={11} className="text-zinc-500" /> City <span className="text-red-500">*</span>
                      </label>
                      <input
                        type="text"
                    ...
                    <div>
                      <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-1.5 flex items-center gap-1.5">
                        <Notebook size={11} className="text-zinc-500" /> Custom Notes / Requests (Optional)
                      </label>
                      <textarea
                    ...
                    <div className="space-y-2">
                      <label className="block text-[10px] font-bold text-zinc-400 uppercase tracking-wider flex items-center gap-1.5">
                        <CreditCard size={11} className="text-zinc-500" /> Payment Option
                      </label>
                      
                      {/* Preselected Cash on Delivery block */}
                      <div className="p-3.5 rounded-xl border border-violet-500/50 bg-violet-600/5 flex items-center justify-between relative overflow-hidden group">
                        <div className="flex items-center gap-3">
                          <div className="w-2.5 h-2.5 rounded-full bg-violet-500 border border-white/20" />
                          <div>
                            <p className="text-xs font-bold text-zinc-100 flex items-center gap-2">
                              Cash on Delivery (COD)
                              <span className="text-[8px] bg-violet-600/30 text-violet-300 border border-violet-500/30 font-black px-1.5 py-0.5 rounded uppercase tracking-wider">
                                Pay when you receive
                              </span>
                            </p>
                            <p className="text-[10px] text-zinc-500 mt-0.5">Pay standard cash upon physical carrier delivery.</p>
                          </div>
                        </div>
                      </div>
                    </div>
            ...
            {items.length > 0 && (
              <div className="p-5 sm:p-6 border-t border-zinc-900 bg-zinc-950 space-y-4">
                {/* Cost breakups */}
                <div className="space-y-2">
                  ...
                  <div className="flex justify-between text-xs text-zinc-400">
                    ...
                    <span className="font-semibold text-zinc-200">Rs. {shippingCost}</span>
                  </div>
                  
                  {/* Glowing Total */}
                  <div className="flex justify-between items-baseline pt-2 border-t border-zinc-900">
                    <span className="text-xs font-bold text-zinc-200 uppercase tracking-wider">Estimated Total</span>
                    <span className="text-xl font-black text-violet-400 drop-shadow-[0_0_15px_rgba(139,92,246,0.35)]">
